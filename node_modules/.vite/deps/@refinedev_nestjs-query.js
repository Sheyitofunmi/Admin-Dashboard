...
var w = (i) => {
  ...
  return visit(i, { Field: { enter(s) {
    if (n) {
      n = false;
      let u = _(s) ?? s;
      if (typeof u.selectionSet > "u") throw new Error("Operation must have a selection set");
...
var M = (i, { callback: r, params: n, meta: e }, t) => {
  let s = { created: I, updated: E, deleted: P }, { resource: o, filters: u, subscriptionType: l, id: a, ids: d } = n ?? {}, f = s[t], { operation: c, query: v, variables: R, operationName: N } = f({ ids: d, id: a, resource: o, filters: u, meta: e, subscriptionType: l }), Q = (V) => {
    r(V.data[c]);
  };
  return i.subscribe({ query: v, variables: R, operationName: N }, { next: Q, error: console.error, complete: () => null });
};
var D = { eq: "eq", ne: "neq", lt: "lt", gt: "gt", lte: "lte", gte: "gte", in: "in", nin: "notIn" };
var k = (i, r) => {
  if (i === "contains") return { iLike: `%${r}%` };
  if (i === "ncontains") return { notILike: `%${r}%` };
  if (i === "containss") return { like: `%${r}%` };
  if (i === "ncontainss") return { notLike: `%${r}%` };
  if (i === "startswith") return { iLike: `${r}%` };
  if (i === "nstartswith") return { notILike: `${r}%` };
  if (i === "startswiths") return { like: `${r}%` };
  if (i === "nstartswiths") return { notLike: `${r}%` };
  if (i === "endswith") return { iLike: `%${r}` };
  if (i === "nendswith") return { notILike: `%${r}` };
  if (i === "endswiths") return { like: `%${r}` };
  if (i === "nendswiths") return { notLike: `%${r}` };
  if (i === "null") return { is: null };
  if (i === "nnull") return { isNot: null };
  if (i === "between") {
...
var $ = (i) => {
  let r = {};
  return i.filter((n) => Array.isArray(n.value) && n.value.length === 0 ? false : typeof n.value == "number" ? Number.isFinite(n.value) : n.value != null).map((n) => n.operator === "and" || n.operator === "or" ? (0, import_set.default)(r, n.operator, [$(n.value)]) : "field" in n ? (0, import_set.default)(r, n.field, k(n.operator, n.value)) : {}), r;
};
var h = (i) => i.map((r) => ({ field: r.field, direction: r.order.toUpperCase() }));
var B = (i) => {
  if (i.mode === "off") return { limit: 2147483647 };
  if (i.mode === "server" && !(!i.current || !i.pageSize)) return { limit: i.pageSize, offset: (i.current - 1) * i.pageSize };
};
...
var A = (i) => ({ getList: async ({ resource: r, pagination: n, sorters: e, filters: t, meta: s }) => {
  ...
  if (s != null && s.gqlQuery) a = s == null ? void 0 : s.gqlQuery, d = { filter: t ? $(t) : {}, sorting: e ? h(e) : [], paging: u };
  else {
  ...
  if (e != null && e.gqlQuery) return { data: (await i.request(e.gqlQuery, { filter: { id: { in: n } } }))[t].nodes };
  ...
  if (s) return { data: (await i.request(s, { input: { [(0, import_camelcase.default)((0, import_pluralize.singular)(r))]: n } }))[t] };
  ...
  if (o) return { data: (await i.request(o, { input: { [(0, import_camelcase.default)(r)]: n } }))[s] };
  ...
  if (o) return { data: (await i.request(o, { input: { id: n, update: e } }))[s] };
  ...
  if (e != null && e.gqlMutation) return { data: (await i.request(e.gqlMutation, { input: { id: n } }))[s] };
  let o = lib_default`
  ...
  if (s) return { data: await i.request(s, (t == null ? void 0 : t.variables) ?? {}) };
  if (t != null && t.rawQuery) return { data: await i.request(t.rawQuery, t.variables) };
  if (t) {
    if (t.operation) {
      let o, u;
      if (n === "get") {
...
var te = (i) => {
  ...
  return { subscribe({ callback: n, params: e, meta: t }) {
    ...
    if (!t || !o || !s) throw new Error("[useSubscription]: `meta`, `subscriptionType` and `resource` are required in `params` for graphql subscriptions");
    let u = [];
